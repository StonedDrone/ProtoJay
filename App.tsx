...
import { type Shape, type Layer, type GlobalEffects, type Scene, type Point, type VisualType, blendModes } from './types';
...
const initialShapes: Shape[] = [
    { id: 'shape-1', points: [{x: 150, y: 150}, {x: 350, y: 150}, {x: 350, y: 350}, {x: 150, y: 350}], visual: 'gradient-purple' },
    { id: 'shape-2', points: [{x: 450, y: 200}, {x: 650, y: 200}, {x: 550, y: 400}], visual: 'grid' }
];

const initialLayers: Layer[] = [
    { id: 'layer-1', shapeId: 'shape-1', name: 'Main Square', opacity: 1, blendMode: 'normal', visible: true },
    { id: 'layer-2', shapeId: 'shape-2', name: 'Accent Triangle', opacity: 0.8, blendMode: 'screen', visible: true }
];
...
const App: React.FC = () => {
    ...
    const updateShape = useCallback((id: string, newPoints: Point[]) => {
        setShapes(prev => prev.map(s => s.id === id ? { ...s, points: newPoints } : s));
    }, []);

    const addShape = (visual: VisualType) => {
    ...
    const visuals: { name: string, type: VisualType }[] = [
        ...
        <div className="h-screen w-screen bg-gray-900 text-gray-200 flex flex-col overflow-hidden">
            ...
            <div className="flex-grow grid grid-cols-12 grid-rows-1 gap-2 p-2 h-full overflow-hidden">
                ...
                <div className="col-span-3 flex flex-col gap-2 overflow-y-auto">
                    <Panel title="Visuals Library">
                        <div className="grid grid-cols-2 gap-2">
                            {visuals.map(v => (
                                <button key={v.type} onClick={() => addShape(v.type)} className="bg-gray-700 hover:bg-indigo-500 rounded p-2 text-sm transition-colors duration-150 text-center">
                                    {v.name}
                                </button>
                            ))}
                        </div>
                    ...
                    <Panel title="Layers" flex>
                        ...
                        {layers.map(layer => (
                            <div key={layer.id} onClick={() => setSelectedLayerId(layer.id)} className={`p-2 rounded cursor-pointer transition-colors ${selectedLayerId === layer.id ? 'bg-indigo-600' : 'bg-gray-700 hover:bg-gray-600'}`}>
                                ...
                                <div className="text-xs text-gray-400">{shapes.find(s => s.id === layer.shapeId)?.visual}</div>
                            </div>
                        ))}
                        </div>
                    </Panel>
                ...
                <div className="col-span-6 flex flex-col gap-2 h-full">
                    <Panel title="Projection Output" flex>
                       <Canvas shapes={shapes} layers={layers} effects={effects} updateShape={updateShape} selectedShapeId={selectedShape?.id || null} />
                    </Panel>
                </div>
